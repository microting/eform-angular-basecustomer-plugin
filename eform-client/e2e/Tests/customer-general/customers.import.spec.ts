...
describe('Customers plugin import page', function () {
  ...
  it('should import customers list', function () {
    ...
    expect(
      customersN,
      'After choosing file, two customers must appear in table'
    ).equal(3);
    ...
    expect(
      customersAfter,
      'Number of customers is not bigger than before'
    ).greaterThan(customersBefore);
  });
  it('should not import customers', function () {
    const localPath = process.cwd();
    const customersBefore = customersImportPage.numberOfCustomers;
    customersPage.importCustomerBtn.click();
    ...
    expect(customersAfter, `Number of customers is't same as before`).equal(
      customersBefore
    );
  });
  after(function () {
