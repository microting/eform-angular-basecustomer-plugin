...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("eFormAPI/Plugins/Customers.Pn/Customers.Pn")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormBaseCustomerBase'
        ]
        process_repository('Customers.Pn.csproj', packages, 'eform-angular-basecustomer-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("../../../..")
            run_command(f"github_changelog_generator -u microting -p eform-angular-basecustomer-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
